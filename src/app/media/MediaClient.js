...
export default function MediaClient({ sanityImages = [] }) {
  const [lightboxOpen, setLightboxOpen] = useState(false);
  const [lightboxIndex, setLightboxIndex] = useState(0);

  const activeGallery = sanityImages.length > 0 
    ? sanityImages.map(img => ({ src: img.imageUrl, label: img.title })) 
    : galleryImages;
        ...
        <div className={styles.lightbox} onClick={closeLightbox}>
          <div className={styles.lightboxContent} onClick={(e) => e.stopPropagation()}>
            ...
            <Image
              ...
              alt={activeGallery[lightboxIndex].label}
              width={900}
              height={600}
            ...
            <div className={styles.lightboxCaption}>
              {activeGallery[lightboxIndex].label}
            </div>
          </div>
        </div>
      )}
